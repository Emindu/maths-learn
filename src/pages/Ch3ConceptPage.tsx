...
import { getCh3ConceptById } from '../data/ch3ConceptsData';
import { ConceptContent } from './ProbabilityConceptPage';

export const Ch3ConceptPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const concept = id ? getCh3ConceptById(id) : undefined;
    ...
    return (
      <div style={{
        ...
        <p style={{ color: 'var(--color-text-secondary)', marginBottom: 'var(--space-24)' }}>
          No Chapter 3 concept with id "{id}" exists.
        </p>
        <Link
          to="/#ch3concepts"
          style={{
  ...
  return <ConceptContent concept={concept} backHref="/#ch3concepts" />;
};
